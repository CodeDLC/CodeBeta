package fun.vegax.features.impl.misc;

import antidaunleak.api.UserProfile;
import fun.vegax.VegaXDLC;
import fun.vegax.common.discord.DiscordManager;
import fun.vegax.events.chat.ChatEvent;
import fun.vegax.features.module.Module;
import fun.vegax.features.module.ModuleCategory;
import fun.vegax.utils.client.chat.ChatMessage;
import fun.vegax.utils.client.managers.event.EventHandler;
import fun.vegax.utils.client.managers.file.FileRepository;
import fun.vegax.utils.client.managers.file.exception.FileLoadException;
import fun.vegax.utils.client.managers.file.impl.PrefixFile;
import fun.vegax.utils.math.calc.Calculate;
import fun.vegax.utils.math.time.StopWatch;
import lombok.experimental.NonFinal;
import org.apache.logging.log4j.core.appender.rolling.action.IfAll;
import org.lwjgl.glfw.GLFW;
import org.lwjgl.system.MathUtil;

public class SelfDestruct extends Module {
    public static boolean unhooked;

    public SelfDestruct() {
        super("SelfDestruct", "Self Destruct", ModuleCategory.MISC);
    }
    @NonFinal
    StopWatch timer = new StopWatch();

    @Override
    public void activate() {
        unhooked = true;

        VegaXDLC.getInstance().getDiscordManager().stopRPC();

        for (Module module : VegaXDLC.getInstance().getModuleProvider().getModules()) {
            if (module != this && module.isState()) {
                module.setState(false);
            }
        }

        ChatMessage.brandmessage("Для возвращения чита впишите в чат ваш username в чите");
        ChatMessage.brandmessage("Сообщение удалится через пол секунды");
        if (timer.every(3000)) {
            mc.inGameHud.getChatHud().clear(true);
        }

        for (Module module : VegaXDLC.getInstance().getModuleProvider().getModules()) {
            module.setKey(GLFW.GLFW_KEY_UNKNOWN);
        }

        VegaXDLC.getInstance().getCommandDispatcher().prefix = "" + Calculate.getRandom(0, 9999999);

        super.activate();
    }

    @EventHandler
    public void onChat(ChatEvent event) {
        String msg = event.getMessage().trim();
        if (msg.equalsIgnoreCase(UserProfile.getInstance().profile("username"))) {
            unhooked = false;
            VegaXDLC.getInstance().getDiscordManager().setRunning(true);
            state = false;
            VegaXDLC.getInstance().getCommandDispatcher().prefix = ".";
            ChatMessage.brandmessage("Unhook reset to FALSE");
            event.setCancelled(true);
        }
    }

    @Override
    public void deactivate() {
        unhooked = false;
        super.deactivate();
    }
}
