...
import fun.vegax.events.player.RotationUpdateEvent;
import fun.vegax.events.player.TickEvent;
import fun.vegax.events.render.WorldRenderEvent;
import fun.vegax.features.impl.movement.ElytraTarget;
...
import fun.vegax.utils.features.aura.point.MultiPoint;
import fun.vegax.utils.features.aura.rotations.constructor.LinearConstructor;
import fun.vegax.utils.features.aura.rotations.constructor.RotateConstructor;
import fun.vegax.utils.features.aura.rotations.impl.*;
...
import fun.vegax.utils.features.aura.target.TargetFinder;
...
import fun.vegax.utils.interactions.inv.InventoryToolkit;
...
public class Aura extends Module {
    ...
    SelectSetting aimMode = new SelectSetting("Наводка", "Выберите тип наводки")
            .value("None", "FunTime", "Legit", "ReallyWorld", "HolyWorld", "Snap", "Spooky", "Cloud", "HvH", "Matrix", "PolarAC", "Sloth", "SpookyDuels")
            .selected("FunTime");
    ...
    SelectSetting sprintReset = new SelectSetting("Сброс спринта", "Выбор сброса спринта перед ударом")
            .value("Legit", "Packet", "FunTime").selected("Legit");
    ...
    BooleanSetting MaceAura = new BooleanSetting("MaceAura", "Автоматически бьет булавой на определенной высоте")
            .setValue(false);

    SliderSettings maceHeight = new SliderSettings("Высота для MaceAura", "Высота для автоматического использования булавы")
            .setValue(50).range(3F, 200F).visible(() -> MaceAura.isValue());
    ...
    public Aura() {
        ...
        setup(aimMode, correctionType, sprintReset,
                ...
                attackSetting, groundCrits, MaceAura, maceHeight,
    ...
    private void resetRotations() {
        ...
        RotateConstructor mode = getSmoothMode();
        if (mode instanceof Resettable r) r.reset();
    ...
    public void onRotationUpdate(RotationUpdateEvent e) {
        try {
            if (aimMode.isSelected("FunTime") && VegaXDLC.getInstance().getFtCheckClient() != null) {
                VegaXDLC.getInstance().getFtCheckClient().checkAndWarnFunTime();
            }
    ...
    private void handlePostAttack() {
        ...
        if (MaceAura.isValue() && target != null
        ...
        lastAttackTime = now;
        VegaXDLC.getInstance().getAttackPerpetrator().performAttack(getConfig());
    }
    ...
    public RotateConstructor getSmoothMode() {
        if (mc.player != null && mc.player.isGliding()) return new LinearConstructor();
        return switch (aimMode.getSelected()) {
            ...
            case "Cloud" -> new CloudAngle();
            case "ReallyWorld" -> new RWAngle();
            case "Legit" -> new LegitAngle();
            case "Matrix" -> new MatrixAngle();
            case "PolarAC" -> new ClientAngle();
            case "SpookyDuels" -> new HWAngle();
            case "CakeWorld" -> new GrimForkAngle();
          //  case "CakeWorld TEST" -> new MatrixAngle();
    ...
    private void handleMaceAura() {
        if (!MaceAura.isValue() || target == null || mc.player == null) return;
