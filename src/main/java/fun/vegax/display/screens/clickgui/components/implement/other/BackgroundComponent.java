package fun.vegax.display.screens.clickgui.components.implement.other;
...
public class BackgroundComponent extends AbstractComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String point = " • ";
        DiscordManager discord = VegaXDLC.getInstance().getDiscordManager();
        VegaXDLC.getInstance().getScissorManager().push(matrix.peek().getPositionMatrix(), 0, 0, window.getScaledWidth(), window.getScaledHeight());
        ...
        if (MenuScreen.INSTANCE.getCategory() == ModuleCategory.CONFIGS) {
            ...
            ScissorAssist scissorManager = VegaXDLC.getInstance().getScissorManager();
            float listX = x + 43f;
            float listY = y + 65f;
        ...
        VegaXDLC.getInstance().getScissorManager().pop();
    }
        ...
        if (MenuScreen.INSTANCE.getCategory() == ModuleCategory.CONFIGS && button == 0) {
            ...
            for (Map<String, Object> map : displayedConfigs) {
                ...
                if (Calculate.isHovered(mouseX, mouseY, configX + 162, configY + 35, 14, 15)) {
                    if (isDefaultTab) {
                        try {
                            File dir = new File(VegaXDLC.getInstance().getClientInfoProvider().clientDir(), "Custom");
                            File configFile = new File(dir, config + ".json");
                            String json = new String(Files.readAllBytes(configFile.toPath()));
                            File tempFile = new File(VegaXDLC.getInstance().getClientInfoProvider().configsDir(), "temp.json");
                            Files.write(tempFile.toPath(), json.getBytes());
                            VegaXDLC.getInstance().getFileController().loadFile("temp.json");
                            tempFile.delete();
    ...
    private List<Map<String, Object>> getCloudConfigs() {
        try {
            if (!VegaXDLC.getInstance().getCloudConfigClient().isConnected()) {
            ...
            String response = VegaXDLC.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
    ...
    private void createDefaultConfig() {
        ...
        String name;
        do {
            name = "vegaxdlcConfig" + String.format("%03d", random.nextInt(1000));
    ...
    private String getCurrentConfigJson() {
        ...
        try {
            VegaXDLC.getInstance().getFileController().saveFile("temp.json");
    ...
    private void saveCloudConfig(String name) {
        ...
        data.put("avatar_hash", VegaXDLC.getInstance().getDiscordManager().getAvatarId().toString());
    ...
    private void saveCloudConfigWithJson(String name, String json) {
        ...
        try {
            ...
            String message = gson.toJson(request);
            VegaXDLC.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
        } catch (Exception e) {
