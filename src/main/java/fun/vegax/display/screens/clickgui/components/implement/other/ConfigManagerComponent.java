...
public class ConfigManagerComponent extends AbstractComponent {
    ...
    public void refreshConfigs() {
        ...
        if (configFiles != null) {
            for (File file : configFiles) {
                if (file.isFile() && file.getName().endsWith(".code")) {
                    configs.add(file.getName().replace(".code", ""));
                }
            }
        }
    }
    ...
    private void saveConfig() {
        ...
        try {
            ...
            ModuleFile moduleFile = new ModuleFile(
                    VegaXDLC.getInstance().getModuleRepository(),
                    VegaXDLC.getInstance().getDraggableRepository()
            );
            moduleFile.saveToFile(customDir, inputText + ".code");
    ...
    private void loadConfig(String name) {
        try {
            File customDir = new File(VegaXDLC.getInstance().getClientInfoProvider().configsDir(), "Custom");
            File configFile = new File(customDir, name + ".code");
            ...
            if (configFile.exists() && configFile.length() > 0) {
                ...
                moduleFile.loadFromFile(customDir, name + ".code");
