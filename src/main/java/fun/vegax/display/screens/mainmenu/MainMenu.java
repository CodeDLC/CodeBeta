package fun.vegax.display.screens.mainmenu;

import fun.vegax.common.animation.Direction;
import fun.vegax.common.animation.implement.Decelerate;
import fun.vegax.display.screens.mainmenu.altscreen.AltScreen;
import fun.vegax.utils.client.text.TextAnimation;
import fun.vegax.utils.display.color.ColorAssist;
import fun.vegax.utils.display.font.Fonts;
import fun.vegax.utils.display.gif.GifRender;
...
public class MainMenu extends Screen implements QuickImports {
    ...
    private void renderHeader(DrawContext ctx, int a, float slide) {
        ...
        if (glowAlpha > 1) {
            Fonts.getSize(22, Fonts.Type.VEGAXDLC_TITLE)
                    .drawCenteredString(ctx.getMatrices(), "CodeDLC",
                            cx, titleY - 0.5f, applyA(C_TITLE_GLOW, glowAlpha));
        }

        // Заголовок
        Fonts.getSize(23, Fonts.Type.VEGAXDLC_TITLE)
                .drawCenteredString(ctx.getMatrices(), "CodeDLC",
    ...
    private void renderFooter(DrawContext ctx, int a) {
        ...
        font10.drawCenteredString(ctx.getMatrices(), "© 2026 CodeDLC Все права защищены.", cx, footerY, c);

        String build = "Build ▸ CodeDLC 1.1 Recode";
