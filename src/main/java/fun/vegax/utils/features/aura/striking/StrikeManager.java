package fun.vegax.utils.features.aura.striking;

import fun.vegax.features.impl.movement.Blink;
import fun.vegax.features.impl.movement.ElytraTarget;
import fun.vegax.utils.client.Instance;
import fun.vegax.utils.client.chat.ChatMessage;
...
import fun.vegax.features.impl.combat.TriggerBot;
import fun.vegax.utils.features.aura.warp.Turns;
import fun.vegax.utils.features.aura.utils.MathAngle;
import fun.vegax.utils.features.aura.utils.RaycastAngle;
import fun.vegax.utils.features.aura.warp.TurnsConnection;
import fun.vegax.utils.features.aura.utils.Pressing;
import fun.vegax.features.impl.movement.AutoSprint;
import fun.vegax.events.item.UsingItemEvent;
import fun.vegax.events.packet.PacketEvent;
import fun.vegax.main.listener.impl.EventListener;
...
import fun.vegax.utils.interactions.simulate.PlayerSimulation;
...
public class StrikeManager implements QuickImports {
    ...
    private void breakShield(StrikerConstructor.AttackPerpetratorConfigurable config) {
        ...
        if (config.isShouldBreakShield() && targetOnShield && axe != null && angle && InventoryFlowManager.script.isFinished()) {
            ...
            ItemStack originalWeapon = mc.player.getMainHandStack();
            
            // Add task to swap to axe and attack
            InventoryFlowManager.addTask(() -> {
                // Swap to axe
                InventoryTask.swapHand(axe, Hand.MAIN_HAND, false);
                // Small delay to ensure swap completes
                InventoryFlowManager.postScript.cleanup().addTickStep(0, () -> {
                    // Attack with axe to break shield
                    attack(config);
                    // Wait a tick then swap back to original weapon
                    InventoryFlowManager.postScript.cleanup().addTickStep(1, () -> {
                        // Swap back to original weapon slot
