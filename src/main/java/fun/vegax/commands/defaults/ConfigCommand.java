package fun.vegax.commands.defaults;
...
import fun.vegax.utils.client.managers.file.FileRepository;
import fun.vegax.utils.client.managers.file.impl.ModuleFile;
import fun.vegax.display.screens.clickgui.components.implement.other.BackgroundComponent;
import fun.vegax.VegaXDLC;
import fun.vegax.main.client.ClientInfoProvider;
import fun.vegax.utils.client.managers.api.command.Command;
import fun.vegax.utils.client.managers.api.command.argument.IArgConsumer;
import fun.vegax.utils.client.managers.api.command.datatypes.ConfigFileDataType;
import fun.vegax.utils.client.managers.api.command.exception.CommandException;
import fun.vegax.utils.client.managers.api.command.helpers.Paginator;
import fun.vegax.utils.client.managers.api.command.helpers.TabCompleteHelper;
import fun.vegax.utils.client.managers.file.FileController;
...
import static fun.vegax.utils.client.managers.api.command.IBaritoneChatControl.FORCE_COMMAND_PREFIX;
...
public class ConfigCommand extends Command {
    ...
    protected ConfigCommand(VegaXDLC main) {
        super("config", "cfg");
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (arg.contains("load")) {
            ...
            if (configFile.exists() && configFile.length() > 0) {
                try {
                    ...
                    ModuleFile moduleFile = new ModuleFile(VegaXDLC.getInstance().getModuleRepository(), VegaXDLC.getInstance().getDraggableRepository());
        ...
        if (arg.contains("save")) {
            ...
            try {
                ...
                var fileController = VegaXDLC.getInstance().getFileController();
    ...
    public List<String> getConfigs() {
        ...
        File customDir = new File(VegaXDLC.getInstance().getClientInfoProvider().configsDir(), "Custom");
    ...
    private void refreshGuiConfigs() {
        ...
        try {
            var menuScreen = fun.vegax.display.screens.clickgui.MenuScreen.INSTANCE;
